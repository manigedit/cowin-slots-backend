...
async function chaljaBhai () {
    ...
    districts.forEach(async (district) => {
        logger.log.info(`DISTRICT: District search started for ${district.district_name}`)
...
function getSlotsbyPincode(pincode, date) {
    logger.info(`PINCODE_SEARCH: statred pincode search for ${pincode} on ${date}`)
    let config = {
        method: 'get',
        ...
        .then(function (slots) {
            ...
            try {
                ...
                processPincodeResponse(slots.data, pincode)
                    ...
                    console.log('Blocked API call for pincode ', pincode, ' at ', new Date());
                }
            } catch (e) {
                console.log(e)
            }
    ...
    if (usersOfthatPincode) {
        usersOfthatPincode.map((user) => {
            ...
            for (let i = 0; i < data.centers.length; i++) {
                ...
                let notifiableSlots = data.centers[i].sessions.filter( slot => slot.min_age_limit <= user.age && slot.available_capacity > 0);
...
function getSlotsForDate(district_id , date, districtName) {
    logger.log.info(`DISTRICT_SEARCH: started district search for ${districtName} on ${date}`)
    let config = {
        method: 'get',
    ...
    axios(config)
        .then(function (slots) {
            ...
            logger.log.info(`DISTRICT_SEARCH_BLOCKED: API blocked at For ${districtName} ` )
        })
        .catch(function (error) {
            logger.log.info(`DISTRICT_SEARCH_ERROR: API blocked at For ${districtName} ` )
        });
}
...
function processDistrictsResponse(district) {
    district.centers.map((center) => {
        let validSlots = center.sessions.filter(slot =>  slot.available_capacity > 0)
...
function tryToNotify(pincode, center, slots) {
    ...
    if (usersOfthatPincode) {
        usersOfthatPincode.map((user) => {
            let notifableSlots = slots.filter(slot => slot.min_age_limit <= user.age);
...
function notifyUserviaWhatsapp(user,center ,slots) {
    logger.log.info(`NOTIFY_SLOT: notified ${user.phone} for ${center} with ${slots.length} slots`)
    ...
    for (let i = 0; i < usersArray.length; i++)  {
        redisClient.get(`${usersArray[i].phone}-welcomed`, (err, data) => {
            ...
            if (data != null) {
                ...
                redisClient.set(`${usersArray[i].phone}-welcomed`,new Date() ,(err, data)=> {
                    logger.log.info(`NEW_USER: sent welcome message to ${usersArray[i].phone}`)
